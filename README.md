...
## Download
This website can be downloaded & browsed locally using a browser. Go to the repository [mcacc-collars/website](https://github.com/mcacc-collars/website) and look for the `<> Code` drop-down menu which contains a "download.zip" option. This is [the direct link](https://github.com/mcacc-collars/website/archive/refs/heads/main.zip) to download.

## Directories explained
- `/docs` contains the working website. Browse the site offline there (start at `/docs/index.html`).

- `/src` contains the working (permanent) .html, .css & .js files (with comments, indentation). Those files end up in `/docs` as minified copies (the pages served to visitors, their page source not very human readable). 
